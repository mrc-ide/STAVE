...
file_path <- system.file("extdata", "vignettes/encoding_example_01.xlsx", package = "STAVE")
...
```{r, echo=FALSE}
input_counts |>
  filter(study_id == "study_03") |>
  kbl(format = "html", table.attr = "style='width:100%; white-space: nowrap;'") |>
  kable_styling(bootstrap_options = c("hover", "condensed"), 
                full_width = TRUE)
```

See the [variant string](https://github.com/mrc-ide/variantstring) package for further details of how to encode mixed calls, but in short, the `/` symbol specifies an un-phased mixed call, and `|` a phased mixed call.
...
Overall, there is a balance to strike between capturing everything and capturing enough. This balance will vary from one application to the next. In general, if your aim is to capture genome-wide haplotypes then STAVE is probably not the best solution. You may be better sticking with other common formats like [VCF](https://samtools.github.io/hts-specs/VCFv4.2.pdf) and [PMO](https://plasmogenepi.github.io/PMO_Docs/). If your aim is to capture key drug resistance combinations in a lightweight and flexible notation, then STAVE may be a good choice.

---

The [next page](articles/howto_calculating_prevalence.html) goes into the details of how prevalence is calculated from the encoded data.
